...
const startTranscriptionJob = async (voiceNoteId, userId) => {
	...
	try {
		voiceNoteInstance = await voiceNoteModel.findOne({
			_id: voiceNoteId,
			userId: userId,
		});
		...
		console.error(`[Transcription Error] voiceNoteId=${voiceNoteId}:`, error.message || error);
		// Mark voice note as failed so the UI can show the correct status
		if (voiceNoteInstance) {
			try {
				voiceNoteInstance.status = "FAILED";
				await voiceNoteInstance.save();
