    ...
    return async (req, res, next) => {
        try {
            ...
            if(attempts && Number(attempts) >= limit){
                return res.status(429).json({success: false, message: `Rate limit exceeded. Try again after ${Math.ceil(window / 60)} minutes.`});
            }
        ...
        return next();
            ...
            return res.status(500).json({success: false, message: "Internal RateLimiting Error"});
        }
